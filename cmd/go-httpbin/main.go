...
func main() {
	...
	// Define command-line flags and link them to the config fields.
	flag.IntVar(&cfg.port, "port", 8080, "listening port")
	flag.StringVar(&cfg.headerKeys, "logheaders", "Authentication,Connection", "comma-separated list of headers to log, e.g. Authentication,X-Forwarded-For")
	...
	h := httpbin.New(
		...
		httpbin.WithObserver(func(ctx context.Context, result httpbin.Result) {
			...
			hAttrs = append(hAttrs, slog.Int("http.status_code", result.Status))
			hAttrs = append(hAttrs, slog.String("http.method", result.Method))
			for _, hkey := range strings.Split(cfg.headerKeys, ",") {
				hAttrs = append(hAttrs, slog.String(fmt.Sprintf("header[%s]", hkey), strings.Join(result.ReqHeaders[hkey], "\n")))
			}
			hAttrs = append(hAttrs, slog.String("http.uri", result.URI))
			hAttrs = append(hAttrs, slog.Int64("http.response_size_bytes", result.Size))
			hAttrs = append(hAttrs, slog.Int64("http.request_size_bytes", result.RequestSize))
			hAttrs = append(hAttrs, slog.String("http.user_agent", result.UserAgent))
			hAttrs = append(hAttrs, slog.String("network.client_ip", result.ClientIP))
			hAttrs = append(hAttrs, slog.Float64("duration_ms", durationMS))
